...
# Example: Complete Palawan change detection workflow
def example_palawan_change_detection():
    """
    Example: Detect changes in Palawan from 2020 to 2024
    """
    
    # Define study area
    palawan = ee.Geometry.Rectangle([118.5, 9.5, 119.5, 10.5])
    
    # Load pre-classified images (assumes you have these)
    # In practice, these would be the output from Session 1 classification
    classification_2020 = ee.Image('users/your_username/palawan_lulc_2020')
    classification_2024 = ee.Image('users/your_username/palawan_lulc_2024')
    ...
    # Detect forest loss
    forest_loss = detect_forest_loss(classification_2020, classification_2024, [1, 2])
    
    # Identify hotspots
    hotspots = detect_deforestation_hotspots(forest_loss, palawan, kernel_radius=1000)
    
    # Create change matrix
    change_matrix = create_change_matrix(classification_2020, classification_2024)
    
    # Export results
    export_change_map(forest_loss, palawan, 'palawan_forest_loss_2020_2024')
    export_change_map(hotspots, palawan, 'palawan_deforestation_hotspots_2020_2024')
    export_change_map(change_matrix, palawan, 'palawan_change_matrix_2020_2024')
...
1. TEMPORAL ALIGNMENT:
   - Use same season for both years
   - Match phenological stages
   - Account for inter-annual variability
   - Consider cloud cover differences

2. CLASSIFICATION CONSISTENCY:
   - Use same training data approach
   - Same features and parameters
   - Same classification algorithm
   - Validate both classifications independently

3. MINIMUM MAPPING UNIT:
   - Set threshold for meaningful change
   - Typically 0.5-1 hectare for 10m data
   - Filter out small patches (noise)
   - Use sieve filter if needed

4. CHANGE VALIDATION:
   - Sample stratified by change type
   - Use high-resolution imagery
   - Field validation if possible
   - Calculate change detection accuracy

5. REPORTING:
   - Report gross changes (gains + losses)
   - Net changes can hide important dynamics
   - Include transition matrices
   - Visualize hotspots

6. COMMON ERRORS:
   - Classification errors propagate to change
   - Seasonal differences mistaken for change
   - Cloud artifacts
   - Geometric misregistration

7. PALAWAN-SPECIFIC CONSIDERATIONS:
   - Mining: rapid, distinct changes
   - Agriculture: seasonal vs permanent
   - Mangroves: tidal stage sensitivity
   - Urban: gradual expansion patterns

8. DEFORESTATION DRIVERS (Palawan context):
   - Mining and quarrying
   - Agricultural expansion (coconut, rice)
   - Infrastructure development
   - Illegal logging
   - Natural disasters (typhoons)

9. MONITORING RECOMMENDATIONS:
   - Annual updates during dry season
   - Rapid response for known threats
   - Protected area boundary monitoring
   - Coastal zone change tracking

10. STAKEHOLDER COMMUNICATION:
    - Use clear visualizations
    - Quantify in familiar units (hectares)
    - Highlight priority areas
    - Provide actionable recommendations
"""

# Utility function for creating transition matrix table
